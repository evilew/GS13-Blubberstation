...
/datum/controller/subsystem/ticker/proc/declare_completion(was_forced = END_ROUND_AS_NORMAL)
	...
	*/
	for(var/channel_tag in CONFIG_GET(str_list/channel_announce_end_game))
		send2chat("The current round has ended. Please standby for your shift interlude GATO News Network's report!", channel_tag)//GS13 EDIT - GATOFICATION
		//Original: send2chat("The current round has ended. Please standby for your shift interlude Nanotrasen News Network's report!", channel_tag)
